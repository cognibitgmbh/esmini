...
TEST(GetOSIRoadLaneTest, lane_no_obj)
{
    ...
    SE_FlushOSIFile();
    ASSERT_EQ(stat("gt.osi", &fileStatus), 0);
    EXPECT_EQ(fileStatus.st_size, 71736);  // initial OSI size, including static content
    ...
    SE_FlushOSIFile();
    ASSERT_EQ(stat("gt.osi", &fileStatus), 0);
    EXPECT_EQ(fileStatus.st_size, 72275);  // slight growth due to only dynamic updates

    SE_StepDT(0.001f);  // Step for write another frame to osi file
    SE_FlushOSIFile();
    ASSERT_EQ(stat("gt.osi", &fileStatus), 0);
    EXPECT_EQ(fileStatus.st_size, 72815);  // slight growth due to only dynamic updates
...
TEST(GroundTruthTests, check_GroundTruth_including_init_state)
{
    ...
    ASSERT_EQ(stat("gt.osi", &fileStatus), 0);
    EXPECT_EQ(fileStatus.st_size, 19411);
